...
import org.example.util.StringSerializer;
...
@Slf4j
public class ZKSvcManager {

    private ZkClient zkClient;

    public ZKSvcManager(String hostPort) {
    ...
    public void createPersistenceParent() {
        if (!zkClient.exists(PERSISTENCE_NODE)) {
            zkClient.create(PERSISTENCE_NODE, "node used for permanent storage", CreateMode.PERSISTENT);
        }
    }

    public void addToPersistence(String nodeName, String data) {
//        if (!zkClient.exists(PERSISTENCE_NODE)) {
//            // TODO: Consider if this needs to be checked, can just catch create exception
//        }
        zkClient.create(PERSISTENCE_NODE.concat("/").concat(nodeName), data, CreateMode.PERSISTENT);
    }

    public void removeFromPersistence(String nodeName) {
        zkClient.delete(PERSISTENCE_NODE.concat("/").concat(nodeName));
    }

    public void updatePersistence(String nodeName, String data) {
        zkClient.writeData(PERSISTENCE_NODE.concat("/").concat(nodeName), data);
    }
