...
@Slf4j
public class DataLayerMgrRDB extends DataLayerMgrBase implements AutoCloseable {
...
//    Map<String, Integer> watchManager;
    ...
    // TODO: Consider moving multiplier to base
...
//    CoordinationSvcCacheLayer<String, LogRecord> recordDataCache;
    ...
    public DataLayerMgrRDB(Object credentials, String zNodeRoot)
...
//        backgroundLogReaderTest.shutdown();
            try {
                ...
                if (!backgroundExecutor.awaitTermination(20, TimeUnit.MILLISECONDS)) {
...
//            System.out.println(String.format("Successful connection."));
    ...
    public static void testCleanup() {
        ...
        String sqlTimeoutOwnerTable = "DROP TABLE IF EXISTS timeout_owner_table";
        String sqlWatchTable = "DROP TABLE IF EXISTS watch_table";
        String sqlLeaseTable = "DROP TABLE IF EXISTS lease_table";
        String sqlLeaseWaitlistTable = "DROP TABLE IF EXISTS lease_claim_table";
        ...
        try (Connection conn = ConnectionPool.getConnection()) {
            ...
            conn.createStatement().execute(sqlTimeoutOwnerTable);
    ...
    void initializeMetadata(String cSvcNodeRoot) {
        ...
        pathToRowIndex.put(cSvcNodeRoot, 1);
    }
    ...
    public ManagerLeaseHandlerBase startLeaseHandler() {
        return new DBLeaseHandler(this, backgroundLeaseExtender);
    }
    ...
    private void initialize(String zNodeRoot) throws IOException, InvalidServiceStateException {
        ...
        DBMaintenanceRunnable maintenanceRunnable = new DBMaintenanceRunnable(this);
//        backgroundLogReader.scheduleAtFixedRate(logReaderRunnable, 1500, 500, TimeUnit.MILLISECONDS);
        ...
        if (DebugConstants.LOGREADER_FLAG) {
            ...
            backgroundTasksHandler.scheduleAtFixedRate(
                    maintenanceRunnable, 50, 127, TimeUnit.MILLISECONDS);
        }
//        backgroundLogReader.scheduleAtFixedRate(
...
//        " ON CONFLICT (id) DO UPDATE SET type = excluded.type;";
        ...
        String sqlTimeoutOwnerTable = "CREATE TABLE IF NOT EXISTS timeout_owner_table " +
                "(owner_id INT8 PRIMARY KEY NOT NULL)";

        // TODO: watch table
        // TODO: add permanent watch
        // TODO: Consider if can add to index
        // TODO: Clean up watch_table
...
//                " PRIMARY KEY (path, owner_id, transaction_id))";
        ...
        String sqlWTIndex = "CREATE INDEX IF NOT EXISTS owner_idx_wt ON" +
                " watch_table (owner_id) INCLUDE (path, watch_type)";
        ...
        String sqlLeaseWaitlistTable = "CREATE TABLE IF NOT EXISTS lease_claim_table " +
                "(ticket INT NOT NULL," +
                " owner_id INT8 NOT NULL," +
                " expire_timestamp TIMESTAMP WITH TIME ZONE NOT NULL," +
                " PRIMARY KEY (ticket) INCLUDE (owner_id, expire_timestamp))";
//maintenance_role_table
...
//        try (Connection conn = ConnectionPool.getConnection();) {
            ...
            stmt.addBatch(sqlTimeoutOwnerTable);
...
//                System.out.printf("\t\tREMOVE ME - committolog - validate commit [path=%s]\n", path);
                    ...
                    if (logRecord.getType() == CREATE
                        ...
                        if (logRecord.getType() == CREATE
                            ...
                            stmt = conn.prepareStatement("INSERT INTO record_stats_table" +
                                    ...
                                    " child_list_transaction_id, creation_timestamp, mutation_timestamp, data_length," +
...
//                            stmt.setString(1, logRecord.getPath());
                            ...
                            stmt.setInt(7, logRecord.getData().length);
//                            stmt.setInt(7, 0);
//                            stmt.setInt(8, 0);
//                            stmt.setInt(9, 0);
                            if (logRecord.getType() == CREATE) {
                                stmt.setLong(8, NONEPHEMERAL_OWNER_VALUE);
                            } else {
...
//                                        logRecord.getPath(), logRecord.getManagerId()));
                                ...
                                stmt.setLong(8, logRecord.getManagerId());
...
//                            }
                            ...
                            stmt.setInt(3, logRecord.getData().length);
                        ...
                        if (!Objects.isNull(rsEphemDeletes) && rsEphemDeletes.next()) {
                            ...
                            if (ownerCount == 0) {
                                ...
                                conn.createStatement().executeUpdate(String.format(
                                        ...
                                        " WHERE owner_id = %d" +
                                        " AND touch_timestamp < CURRENT_TIMESTAMP - interval '%d seconds'",
                                        ownerId, ephemeralLivenessThreshold.getSeconds()));
                            }
                        }
...
//                    System.out.println("\t\tREMOVE ME - committolog - sync on outstanding changes");
...
//                        watch.stop();
//                        System.out.println(String.format("REMOVE ME - DEBUG - commitToLog
//                        - Waiting on outstandingChangesMonitor & Commit " +
//                                " [execution_time_ms=%s]", watch.getTime(TimeUnit.MILLISECONDS)));
//                    }
                    return true;
                } else {
...
//        try (Connection conn = ConnectionPool.getConnection()) {
    ...
    private void tbdTimeoutManagers(Set<Long> removedOwners) {
        if (backgroundExecutor.isShutdown() || backgroundExecutor.isTerminated() ) {
            ...
            // TODO: Throw invalidservicestate
        }
        for (Long managerId : removedOwners) {
            backgroundExecutor.submit(() -> {
                PreparedStatement stmtEph;
                int retryCount = 0;
                int retryMax = 5;
                while (retryCount < retryMax) {
                    ...
                    try (Connection conn = ConnectionPool.getConnection()) {
                        ...
                        stmtEph = conn.prepareStatement("INSERT INTO timeout_owner_table " +
                                "(owner_id) VALUES (?) ON CONFLICT DO NOTHING");
                        stmtEph.setLong(1, managerId);
                        stmtEph.executeUpdate();
                        stmtEph = conn.prepareStatement("DELETE FROM ephemeral_owner_table WHERE owner_id = ?");
                        stmtEph.setLong(1, managerId);
                        stmtEph.executeUpdate();
                        conn.commit();
                        break;
                        ...
                        if (retryCount == retryMax) {
                            // NOTE: Failed to reflect manager timeout
                            return;
                        }
                    }
                }
...
//        // TODO: remove dryflag
    ...
    public boolean removeWatch(String path, WatchedEvent.WatchType type) throws InvalidServiceStateException {
        try (Connection conn = ConnectionPool.getConnection()) {
            removeWatcherTxHelper(conn, path, type, false);
            return true;
        } catch (SQLException e) {
            throw new InvalidServiceStateException(e);
        }
    ...
    private void removeWatcherTxHelper(Connection conn, String path,
                                       WatchedEvent.WatchType type, boolean txStartedFlag) throws SQLException {
        if (DebugConstants.DEBUG_FLAG) {
            System.out.println(String.format(
                    "REMOVE ME - removeWatcherTxHelper - removing watch [table=log, filepath=%s]", path));
        }
        ...
        if (Objects.isNull(type)) {
            conn.createStatement().executeUpdate(String.format(
                    "DELETE FROM watch_table WHERE path = '%s' AND owner_id = %d",
                    path, managerId));
        } else {
            conn.createStatement().executeUpdate(String.format(
                    "DELETE FROM watch_table WHERE path = '%s' AND owner_id = %d AND watch_type = %d",
                    path, managerId, type.ordinal()));
        }
    ...
    private void addWatcherTxHelper(Connection conn, String path,
                                    ...
                                    boolean txStartedFlag) throws SQLException {
//        Consumer<CommandOutput> watchCallable, boolean txStartedFlag) throws SQLException {
        ...
        if (DebugConstants.DEBUG_FLAG) {
            System.out.println(String.format(
                    "REMOVE ME - addWatcherTxHelper - adding watch [path=%s, type=%s]",
    ...
    private LogRecordStats retrieveStatsHelper(Connection conn, String path) throws SQLException, InterruptedException {
        ...
        if (rs.next()) {
            ...
            stats = new LogRecordStats(
                    ...
                    rs.getInt("data_length"), rs.getLong("ephemeral_owner_id"));
...
//                        "(owner_id) VALUES (?) ON CONFLICT DO NOTHING");
                PreparedStatement stmtEph = conn.prepareStatement("INSERT INTO ephemeral_owner_table (owner_id)" +
                        " SELECT ? WHERE NOT EXISTS (SELECT 1 FROM timeout_owner_table WHERE owner_id = ?)" +
                        " ON CONFLICT DO NOTHING");
                stmtEph.setLong(1, managerId);
                stmtEph.setLong(2, managerId);
                // TODO: consider better naming conventions
                try {
    ...
    public boolean extendSoftLease(Connection conn) throws InvalidServiceStateException {
        try {
            // TODO: Add lease multiplier
//            int extensionLength = 5*500;
...
//            throw new RuntimeException(e);
    ...
    private boolean validateExecutorState() {
        return !isRunning || !((backgroundExecutor.isShutdown() || backgroundExecutor.isTerminated())
                || (backgroundLeaseClaimer.isShutdown() || backgroundLeaseClaimer.isTerminated())
                || (backgroundLeaseExtender.isShutdown() || backgroundLeaseExtender.isTerminated())
                || (backgroundLogReader.isShutdown() || backgroundLogReader.isTerminated()));
    }

    public Future<Boolean> submitSoftLeaseClaim() {
        return backgroundLeaseClaimer.submit(new DBLeaseClaimer(this));
    }
...
//                    " WHERE lease_table.expire_timestamp < CURRENT_TIMESTAMP"
                ...
                // INSERT INTO lease_claim_table (ticket, owner_id, expire_timestamp) VALUES ((SELECT COALESCE(MAX(ticket), 0) + 1 FROM lease_claim_table), 534, CURRENT_TIMESTAMP + interval '2 seconds') RETURNING ticket
                conn.prepareStatement(String.format("DELETE FROM lease_claim_table" +
                        " WHERE expire_timestamp < CURRENT_TIMESTAMP")).executeUpdate();

                int leaseLength = 6 * tbdLeaseMultiplier;
                PreparedStatement stmt = conn.prepareStatement(String.format(
                        "INSERT INTO lease_claim_table (ticket, owner_id, expire_timestamp)" +
                            " VALUES ((SELECT COALESCE(MAX(ticket), 0) + 1 FROM lease_claim_table), ?," +
...
//    }
    ...
    public Future<ManagerLeaseHandlerBase> claimSoftLeaseHandler(boolean highPriorityFlag) {
        CSvcUtils.PrioritizedTask<ManagerLeaseHandlerBase> task = new CSvcUtils.PrioritizedTask<>(
                highPriorityFlag ? 1 : 0,
                new TbdCombinedDBLeaseClaimer(this));
...
//            int leaseLength = 500;
            int leaseLength = 6 * tbdLeaseMultiplier;
            PreparedStatement stmt = conn.prepareStatement(String.format(
                "INSERT INTO lease_table (owner_id, expire_timestamp) VALUES" +
                " (?, CURRENT_TIMESTAMP + interval '%d milliseconds') ON CONFLICT ON CONSTRAINT pk_lt DO UPDATE" +
                " SET owner_id = ?, expire_timestamp = CURRENT_TIMESTAMP + interval '%d milliseconds'" +
                " WHERE lease_table.expire_timestamp < CURRENT_TIMESTAMP", leaseLength, leaseLength));
            stmt.setLong(1, managerId);
            stmt.setLong(2, managerId);
            int rowCount = stmt.executeUpdate();
...
//                        " VALUES (?, CURRENT_TIMESTAMP + interval '%d milliseconds') ON CONFLICT ON CONSTRAINT pk_lt DO UPDATE" +
//                        " SET owner_id = ?, expire_timestamp = CURRENT_TIMESTAMP + interval '%d milliseconds'" +
//                        " WHERE lease_table.expire_timestamp < CURRENT_TIMESTAMP", leaseLength, leaseLength));
//        stmt.setLong(1, managerId);
//        stmt.setLong(2, managerId);
        ...
        while (attemptCount < maxRetry) {
            try (Connection conn = ConnectionPool.getConnection()) {
                ...
                PreparedStatement stmt = conn.prepareStatement("INSERT INTO lease_table (owner_id, expire_timestamp)" +
                        " VALUES (?, CURRENT_TIMESTAMP + interval '2 milliseconds') ON CONFLICT ON CONSTRAINT pk_lt DO UPDATE" +
                        " SET owner_id = ?, expire_timestamp = CURRENT_TIMESTAMP + interval '2 milliseconds'" +
                        " WHERE lease_table.expire_timestamp < CURRENT_TIMESTAMP");
                stmt.setLong(1, managerId);
                stmt.setLong(2, managerId);
                int rowCount = stmt.executeUpdate();
...
//                System.out.println(String.format("DEBUG - getlogtimestamp success [logCount=%d].", counter));
    ...
    static class DBLeaseClaimer implements Callable<Boolean> {
        ...
        public DBLeaseClaimer(DataLayerMgrRDB manager) {
//            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - created\n");
...
//            }
            ...
            int updateCount;
            // TODO: Add lease multiplier
            int leaseLength = 6 * manager.tbdLeaseMultiplier;
//            int leaseLength = 6*500;
            int sleepMS = 0;
            int ticket;
//            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - add lease wait\n");
            try (Connection conn = ConnectionPool.getConnection()) {
...
//            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - queue loop\n");
            // TODO: REMOVE TROUBLE SHOOT TRY CATCH
//            int debugThreshold = 20;
//            int debugCounter = 0;
//            try {
            while (manager.isRunning() && !Thread.currentThread().isInterrupted()) {
                try (Connection conn = ConnectionPool.getConnection()) {
                    ...
                    if (queueCount != 0) {
                        stmt = conn.prepareStatement("SELECT COUNT(*) FROM lease_claim_table" +
                                " WHERE ticket < ? AND expire_timestamp > CURRENT_TIMESTAMP");
                        stmt.setInt(1, ticket);
...
//                        debugCounter++;
//                        if (debugCounter == debugThreshold) {
//                            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - In Loop [queue_count=%d]\n",
//                                    queueCount);
//                            debugCounter = 0;
//                        }

                    if (queueCount == 0) {
//                            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - attempt to claim lease\n");
                        // TODO: Attempt to claim lease
                        stmt = conn.prepareStatement(String.format(
                                "INSERT INTO lease_table (owner_id, expire_timestamp)" +
...
//        int leaseLength = 4;
                    stmt = conn.prepareStatement(String.format(
                            "UPDATE lease_claim_table" +
                                    " SET expire_timestamp = CURRENT_TIMESTAMP + interval '%d milliseconds'" +
                                    " WHERE ticket = ? AND owner_id = ?", sleepMS+leaseLength));
                    stmt.setInt(1, ticket);
                    stmt.setLong(2, manager.managerId);
                    if (stmt.executeUpdate() < 1) {
                        // TODO: Throw CRITICAL ERROR as ticket or owner_id are missing
                    ...
                    if (e.getSQLState().equalsIgnoreCase(SERIALIZATION_FAILURE.getState())) {
                        if (DebugConstants.DEBUG_FLAG) {
                            System.out.println(String.format("DEBUG - DBLeaseClaimer " +
                                            "- Expected exception [SQLState=%s]",
                                    e.getSQLState()));
                        }
                    } else {
                        System.out.println(String.format("DBLeaseClaimer - Exception [SQLState=%s]",
...
//                            throw new RuntimeException(e);
            ...
            return false;
//            } finally {
//                System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - returned\n");
//            }
        }
    }

    static class TbdCombinedDBLeaseClaimer implements Callable<ManagerLeaseHandlerBase> {

        DataLayerMgrRDB manager;
//        boolean highPriorityFlag;

        public TbdCombinedDBLeaseClaimer(DataLayerMgrRDB manager) {
//            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - created\n");
            this.manager = manager;
        }
...
//            System.out.printf("\t\tREMOVE ME - DBLeaseClaimer - add lease wait\n");
            try (Connection conn = ConnectionPool.getConnection()) {
                ...
                log.warn(String.format("DBLeaseClaimer - Exception [SQLState=%s]",
                        e.getSQLState()));
...
//            try {
            boolean leaseFlag = false;
            DBLeaseHandler leaseHandler = null;
            while (manager.isRunning() && !Thread.currentThread().isInterrupted()) {
                try (Connection conn = ConnectionPool.getConnection()) {
                    PreparedStatement stmt;
                    ResultSet rs;
...
//                        leaseFlag = stmt.executeUpdate() > 0;
                        ...
                        if (updateCount > 0) {
                            leaseFlag = true;
                            leaseHandler = new DBLeaseHandler(manager, manager.backgroundLeaseExtender);
                            break;
//                            return true;
                        }
                    } else if (queueCount < 3) {
...
//        int leaseLength = 4;
                    ...
                    if (stmt.executeUpdate() < 1) {
                        ...
                        System.out.println(String.format("CRITICAL ERROR - DBLeaseClaimer - Lease Waitlist Lost"));
                        break;
//                        throw new CompletionException(String.format("CRITICAL ERROR - DBLeaseClaimer - Lease Waitlist Lost"));
//                        return false;
                    }
                    conn.commit();
                } catch (SQLException e) {
                    ...
                    // No-Op
                    log.warn(String.format("DBLeaseClaimer - Exception"));
                    log.warn(String.format("===\nSTACKTRACE\n==="), e);
                }
                if (sleepMS > 0) {
                    try {
...
//                            System.out.println(String.format("CRITICAL ERROR - DBLeaseClaimer - Interrupted"));
//                            e.printStackTrace(System.out);
                            log.error("DEBUG - CRITICAL ERROR - DBLeaseClaimer - Interrupted");
                        }
                        Thread.currentThread().interrupt();
                        throw new CompletionException(e);
//                        return false;
...
//        private ExecutorService leaseExtender;
        private DataLayerMgrRDB manager;
        private Future<?> taskFuture;

        public DBLeaseHandler(DataLayerMgrRDB manager, ExecutorService executor) {
            this.manager = manager;
//            leaseExtender = Executors.newSingleThreadExecutor();
        ...
        public void close() {
            ...
            if (DebugConstants.DEBUG_FLAG) {
                log.debug(String.format("DBLeaseHandler - Closing"));
            }
...
//        int port;
        ...
        //        Thread t;
        LogReader(DataLayerMgrRDB manager) {
            this.manager = manager;
...
//                }
                ArrayList<ImmutablePair<String, WatchedEvent.WatchType>> tableDeletes = new ArrayList<>();
...
//                    int txId = rsWatches.getInt("transaction_id");
                    ...
                    tableDeletes.add(new ImmutablePair<>(path, WatchedEvent.WatchType.fromInteger(watchType)));
...
//                    dummyOutput.setTriggerRecordType(triggerType);
//                    dummyOutput.setRecordVersion(recordVersion);
//                    dummyOutput.setPath(path);
//                    dummyOutput.setTriggerTransactionId(triggerTxId);
...
//                    WatchedEvent registeredWatch = manager.pathWatchMap.get(path);
                    ...
                    HashSet<WatchedEvent> watches = manager.pathWatchMap.get(
                            WatchedEvent.WatchType.fromInteger(watchType)).getOrDefault(path, new HashSet<>());
...
//                        registeredWatch.triggerWatch(dummyOutput);
                    boolean deleteTableEntryFlag = true;
...
//                        triggeredWatches.add(registeredWatch);
                        // TODO: consider better options that more strongly couple watch
                        //  and relevant input (ordered map?)
...
//                                        path, manager.managerId, txId));
                            ...
                            // NOTE: If there is at least one permanent watch,
                            //  do not delete from table
                            deleteTableEntryFlag = false;
                        }
                    }
//                    // TODO: test watch removal
                    if (!deleteTableEntryFlag) {
                        tableDeletes.remove(tableDeletes.size()-1);
                    }
...
////                    }
                ...
                for (ImmutablePair<String, WatchedEvent.WatchType> pair : tableDeletes) {
                    manager.removeWatcherTxHelper(conn, pair.getLeft(), pair.getRight(), false);
//                    conn.createStatement().executeUpdate(String.format(
//                            "DELETE FROM watch_table WHERE path = '%s' AND owner_id = %d AND watch_type = %d",
//                            pair.getLeft(), manager.managerId, pair.getRight().ordinal()));
                }
...
//                }
                ...
                for (Map.Entry<WatchedEvent, WatchInput> watchInputEntry : watchInputMap.entrySet()) {
                    if (manager.backgroundExecutor.isShutdown() || manager.backgroundExecutor.isTerminated() ) {
                        System.out.println(String.format(
                                "CRITICAL ERROR - LogReader - Manager Watch Executor Not Running"));
                    }
                    manager.backgroundExecutor.submit(()
...
//        final int heartbeatThreshold = 100;
        LeaseHandlerRunnable(DataLayerMgrRDB manager) {
            this.manager = manager;
//            testFlag = true;
            if (DebugConstants.DEBUG_FLAG) {
    ...
    @Slf4j
    static class DBMaintenanceRunnable implements Runnable {
        DataLayerMgrRDB manager;

        int heartbeatCounter = 0;
        final int heartbeatThreshold = 200;
        DBMaintenanceRunnable(DataLayerMgrRDB manager) {
            this.manager = manager;
            System.out.println(String.format("New MaintenanceRunnable thread"));
        }

        private void manageEphemerals() {
            HashSet<Long> removedOwners = new HashSet<>();
            try (Connection conn = ConnectionPool.getConnection()) {
                Statement tx_stmt = conn.createStatement();
                try {
                    tx_stmt.execute("BEGIN TRANSACTION ISOLATION LEVEL READ COMMITTED");
//                    int updateCount = 0;
                    // TODO: Is retry needed? True retry must be outside transaction
//                    int retryCount = 0;
//                    int retryMax = 5;
//                    while (updateCount == 0 && retryCount < retryMax) {
//                        retryCount++;
                    int updateCount = conn.createStatement()
                            .executeUpdate(String.format(
                                    "UPDATE ephemeral_owner_table SET touch_timestamp = CURRENT_TIMESTAMP" +
                                            " WHERE owner_id = %d",
                                    manager.managerId));
//                    }
                    conn.commit();
                    // NOTE: check count of updated rows, shut down manager if 0
                    if (updateCount == 0) {
                        System.out.println(String.format("CRITICAL ERROR - MaintenanceRunnable " +
                                "- Missing Heartbeat Entry - Terminating [manager_id=%d]", manager.managerId));
...
//                System.out.println(String.format("REMOVE ME - DEBUG - LogReader Ephemeral Check"));
                    // NOTE: Add liveness check into the query
...
//                                    ownerId, livenessDelta.toString()));
                        ...
                        if (livenessDelta.compareTo(manager.ephemeralLivenessThreshold) >= 0) {
                            System.out.println(
                                    String.format("MaintenanceRunnable - Ephemeral Liveness Threshold Crossed" +
                                                    " [ownerId=%d, livenessDelta=%s]",
                                            ownerId, livenessDelta.toString()));
                            // TODO: delete ephemeral nodes
                            removedOwners.add(ownerId);
...
//            } catch (SQLException | InterruptedException e) {
                ...
                if (e.getSQLState().equalsIgnoreCase(SERIALIZATION_FAILURE.getState())) {
                    if (DebugConstants.DEBUG_FLAG) {
                        System.out.println(String.format("DEBUG - MaintenanceRunnable - Expected exception [SQLState=%s]",
                                e.getSQLState()));
                    }
                } else {
                    System.out.println(String.format("MaintenanceRunnable - Exception [SQLState=%s]",
                            e.getSQLState()));
                    System.out.println(String.format("===\nSTACKTRACE\n==="));
                    e.printStackTrace(System.out);
                }
                System.out.println(String.format("===\nMaintenanceRunnable CONTINUING\n==="));
                return;
            } finally {
                // TODO: Delete ephemerals; Consider helper function. UseWatchexecutor?
                //   select from ephemeral_record_table based on owner_id
                //   generatedeletecmd; execute;
                if (!removedOwners.isEmpty()) {
                    manager.tbdTimeoutManagers(removedOwners);
        ...
        public void run() {
            if (heartbeatCounter >= heartbeatThreshold) {
                log.info(String.format("Heartbeat - MaintenanceRunnable Alive [heartbeat_threshold=%d]",
                        heartbeatThreshold));
                heartbeatCounter = 0;
            }
            heartbeatCounter++;
